...
struct process {
    ...
    signal_manager_t signal;  /* 新增：信号管理器 */
};
...
static long do_kill(int pid, int signum) {
    ...
    struct process *target = pm_get(&g_pm, (pid_t)pid);
    if (!target) return -1;
...
static long do_sigaction(int signum, const void *action, void *old_action) {
    ...
    if (old_action) {
        ...
        if (!kold) return -1;
        if (!signal_get_action(&proc->signal, (signal_no_t)signum, kold)) {
            return -1;
        }
    }
    ...
    if (action) {
        ...
        if (!kact) return -1;
        if (!signal_set_action(&proc->signal, (signal_no_t)signum, kact)) {
            return -1;
        }
    }
...
static long do_sigprocmask(uintptr_t mask) {
    struct process *proc = pm_current(&g_pm);
    if (!proc) return -1;
    return signal_update_mask(&proc->signal, mask);
}
...
static long do_sigreturn(void) {
    ...
    if (signal_return(&proc->signal, &proc->ctx.ctx)) {
        return 0;
    }
    return -1;
}
...
static syscall_signal_t signal_impl;
...
static void init_syscall(void) {
    ...
    signal_impl.kill = do_kill;
    signal_impl.sigaction = do_sigaction;
    signal_impl.sigprocmask = do_sigprocmask;
    signal_impl.sigreturn = do_sigreturn;

    syscall_set_io(&io_impl);
    syscall_set_proc(&proc_impl);
    syscall_set_sched(&sched_impl);
    syscall_set_clock(&clock_impl);
    syscall_set_signal(&signal_impl);
}
...
void main(void) {
    ...
    while (1) {
        ...
        if (is_exception(scause) && code == EXCEP_U_ECALL) {
            ...
            switch (sig_ret.type) {
            case SIGNAL_PROCESS_KILLED:
                pm_exit_current(&g_pm, sig_ret.exit_code);
                continue;
            case SIGNAL_PROCESS_SUSPENDED:
                /* 暂停进程（简化处理：当作正常调度） */
